...
[source,bash,role="execute"]
----
oc get keycloak rhsso -n rhsso -w
----

Wait for `READY: true` (this takes 2-3 minutes). Press Ctrl+C to exit.
...
You should see Keycloak pods running and a route URL.
...
[source,bash,role="execute"]
----
bash $HOME/support/setup-keycloak-oidc.sh
----
...
**Option B: Manual setup via Keycloak admin console:**
...
3. You'll see two login options:
   - **kubeadmin** (local admin account)
   - **rhsso** (OIDC provider)
4. Click **rhsso**
5. You'll be redirected to Keycloak
6. Log in with: `developer1` / `OpenShift123!`
7. You'll be redirected back to OpenShift console
...
2. Copy login command and run it
3. Test permissions:
...
You'll see all cluster projects but cannot create resources (cluster-reader is view-only).

**Test as developer1:**

1. Log in to web console as `developer1` / `OpenShift123!` (via rhsso)
2. Copy login command and run it
3. Check accessible projects:
