...
First, log out of the current CLI session:
...
2. Log out if already logged in (top-right → **Log out**)
3. Click **rhsso** and log in as `viewer1` / `OpenShift123!`
4. Click your username (top-right) → **Copy login command** → **Display Token**
5. Copy and paste the `oc login --token=...` command into your terminal
6. Test permissions:
...
1. In the web console, log out (top-right → **Log out**)
2. Click **rhsso** and log in as `developer1` / `OpenShift123!`
3. Click your username (top-right) → **Copy login command** → **Display Token**
4. Copy and paste the `oc login --token=...` command into your terminal
5. Check accessible projects:
