...
Test that RBAC works with OIDC groups by logging in as `developer1`:
...
3. Click **rhsso** and log in as `developer1` / `OpenShift123!`
...
**Log out to restore cluster admin access:**
...
The terminal automatically uses a service account with cluster-admin permissions.
