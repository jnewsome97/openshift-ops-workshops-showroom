...
* Each control plane node = dedicated compute resources
* Significant infrastructure cost just for control planes
...
clusters    demo-hcp             demo-hcp-admin-kubeconfig   Partial    True        False         The hosted control plane is available
----
...
clusters-demo-hcp     data-etcd-0    Bound    pvc-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx   8Gi        RWO            ocs-storagecluster-ceph-rbd   7m
----
...
You'll see the hosted cluster's API server endpoint:

----
server: https://<endpoint>:6443
----

NOTE: The endpoint could be an AWS LoadBalancer URL (e.g., `xxx.elb.amazonaws.com`) or an internal IP (e.g., `10.10.10.x`) depending on your infrastructure configuration.

**This is the hosted cluster's API server** - a separate Kubernetes API running on the management cluster!
...
----
Kubernetes control plane is running at https://<your-endpoint>:6443
----
...
You'll see resource usage for all control plane pods:

----
NAME                                                 CPU(cores)   MEMORY(bytes)
etcd-0                                               46m          202Mi
kube-apiserver-5c9fdc5f78-xxxxx                      63m          960Mi
kube-controller-manager-6786cd98d5-xxxxx             16m          111Mi
openshift-apiserver-5fbb786685-xxxxx                 17m          205Mi
cluster-version-operator-5dd7f69bd-xxxxx             1m           107Mi
... (30+ more pods)
----

NOTE: You'll see 30-40 pods - this is the entire OpenShift control plane running as pods!

**Add up the totals** (use `oc adm top pods -n clusters-demo-hcp --sum`):

* **Total CPU:** ~250-350m (0.25-0.35 CPU cores)
* **Total Memory:** ~3-4GB
...
**HCP uses a fraction of the resources!**
...
Result: ~35-40 pods running (full control plane!)
...
ip-10-0-11-4.us-east-2.compute.internal     4196m        13%      23602Mi         18%
ip-10-0-13-94.us-east-2.compute.internal    2627m        35%      19914Mi         65%
ip-10-0-45-51.us-east-2.compute.internal    2014m        26%      15717Mi         51%
...
----

**The key insight:** Even with 2 hosted control planes running (~70+ pods total), node utilization remains reasonable. The control planes are distributed across your existing worker nodes.

**Compare to traditional:**
- Would need 6 NEW dedicated nodes (3 per cluster)
- 100% of those nodes reserved for control plane only

**HCP advantage:** Control planes share existing infrastructure!

=== Step 11: Cost Benefits

**Why HCP reduces infrastructure costs:**

* **Traditional:** Each cluster requires 3 dedicated control plane nodes
* **HCP:** Control planes run as pods on existing worker nodes

The more clusters you manage, the greater the savings - you're not provisioning new infrastructure for each cluster's control plane.
...
You'll see two different endpoints (e.g., `https://10.10.10.203:6443` and `https://10.10.10.204:6443`).
...
Let's recap what the hands-on demo demonstrated:

✅ **SPEED**
- Traditional cluster creation: 45-60 minutes
- HCP cluster creation: 3-5 minutes

✅ **EFFICIENCY**
- Control planes run as pods on existing infrastructure
- No dedicated nodes required per cluster
- Minimal additional resource overhead

✅ **MULTI-TENANCY**
- Multiple clusters sharing the same management infrastructure
- Each cluster has its own API endpoint
- Complete isolation between clusters

✅ **SCALABILITY**
- Adding more hosted clusters doesn't require new infrastructure
- Management cluster can host many control planes

**Key Takeaway:** HCP enables faster provisioning and more efficient resource usage for multi-cluster scenarios.
...
* Cost savings that scale with cluster count
* Operational simplicity (centralized backup, upgrade, monitoring)

HCP makes sense when you're managing multiple clusters. The more clusters you have, the greater the efficiency gains.
