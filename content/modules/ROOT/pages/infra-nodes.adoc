...
oc get machineset/${CLUSTERNAME}-worker-${ZONENAME} -n openshift-machine-api -o yaml
----
...
export MACHINESET=$(oc get machinesets -n openshift-machine-api -l machine.openshift.io/cluster-api-machine-role=infra -o jsonpath='{.items[0].metadata.name}')
