...
Get the cloud platform this cluster is running on:
...
**Key insight:** This tells OpenShift which cloud provider APIs to use. On Azure, this would show `Azure`. On VMware, `VSphere`.
...
NOTE: If OpenShift Virtualization is installed, you'll also see `devices.kubevirt.io/kvm`, `devices.kubevirt.io/tun`, and `devices.kubevirt.io/vhost-net` in the capacity output.
...
You'll see multiple storage classes (output includes additional columns like RECLAIMPOLICY, VOLUMEBINDINGMODE, etc.):
...
NOTE: You may also see `ocs-storagecluster-ceph-rbd-virtualization` if OpenShift Virtualization is installed.
...
# Note: Use full API group if ACM is installed
oc get machinesets.machine.openshift.io -n openshift-machine-api

# Scale workers
oc scale machineset.machine.openshift.io <name> --replicas=4 -n openshift-machine-api
----
