...
----
ssh -l demo-user bastion.{guid}.{route_subdomain} -o ServerAliveInterval=120
----

When prompted, enter the password from your credentials page. Then become the `ec2-user`:
...
oc scale machineset cluster-wdmpc-zx5kn-worker-us-east-2c --replicas=1 -n openshift-machine-api
----
