...
.OpenShift Pods
image::app-mgmt-basics/openshift_pod_new.png[]
...
### Services
.OpenShift Service
image::app-mgmt-basics/openshift_service_new.png[width=55%]
...
[source,bash,role="execute",subs="attributes+"]
----
oc get endpointslices -l kubernetes.io/service-name=mapit
----
...
mapit-9b5ch      IPv4          8080,9779,8778    10.128.2.94,10.131.0.99,10.129.2.45 + 2 more...      4m32s
----
...
[NOTE]
====
In OpenShift 4.19 and earlier, you could use `oc get endpoints mapit` to view endpoints. Starting with OpenShift 4.20 (Kubernetes 1.33+), the v1 Endpoints API is deprecated in favor of the more scalable EndpointSlice API. The EndpointSlice API provides better performance for services with many endpoints.
====
...
### Background: Routes
.OpenShift Route
image::app-mgmt-basics/openshift_route_new.png[width=55%]
...
By default, the route we created earlier uses HTTP. To make it accessible via HTTPS, we need to secure it with TLS termination:

[source,bash,role="execute",subs="attributes+"]
----
oc patch route mapit -p '{"spec":{"tls":{"termination":"edge","insecureEdgeTerminationPolicy":"Redirect"}}}'
----

This command adds edge TLS termination to the route, which means:

- The router terminates TLS encryption and forwards unencrypted traffic to the pod
- HTTP requests are automatically redirected to HTTPS
- OpenShift provides a default certificate for the route

Feel free to try out the mapit application:

[source,bash,role="execute",subs="attributes+"]
----
oc get route mapit -o jsonpath='{.spec.host}' | sed 's/^/https:\/\//'
----

It should look something like this:

image::app-mgmt-basics/mapit-app-route.png[width=40%]
