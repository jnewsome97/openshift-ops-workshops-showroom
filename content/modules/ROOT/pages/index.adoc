...
You will be interacting with an OpenShift 4 cluster that is running on {ENVIRONMENT}.
...
link:https://github.com/rhpds/showroom[Showroom] is a
...
[source,none,role="execute",subs="attributes+"]
----
echo Hello World\!
----
...
Let's inspect the `ClusterRoleBinding` that gave our
`ServiceAccount` _cluster-admin_ `ClusterRole`:

[source,bash,role="execute",subs="attributes+"]
----
oc get clusterrolebinding -l app=showroom -o yaml
----
...
----
Please note that if you experience any issues with Showroom
throughout this workshop (application is not available), you should try reopening it
in an incognito window after a few minutes.
----

Please set up the workshop support files by running the following commands:

[source,bash,role="execute",subs="attributes+"]
----
git clone -b ocp4-prod https://github.com/OpenShiftDemos/openshift-ops-workshops.git ~/openshift-ops-workshops
----

[source,bash,role="execute",subs="attributes+"]
----
cp -r ~/openshift-ops-workshops/support ~/support
----

Verify the support directory is in place:
