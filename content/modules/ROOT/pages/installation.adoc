...
This workshop provides a pre-installed OpenShift cluster. You're already logged in as `kubeadmin`, the default cluster administrator.
...
You'll see it go from `Pending` to `Bound` (CSI provisioner creates EBS volume automatically).
...
Access Prometheus (read-only query):
...
**Next Module:** Configure LDAP authentication to integrate enterprise user identities.
