...
2. You'll see the **Red Hat Developer Hub** landing page
3. By default, you can access as a guest or login with OpenShift credentials
...
1. In the OpenShift console, go to **Workloads** -> **Deployments**
2. Select the **backstage** project from the project dropdown
3. You'll see the `backstage-developer-hub` deployment
...
[source,bash,role="execute"]
----
oc logs -n backstage -l app.kubernetes.io/component=backstage --tail=50
----
...
[source,bash,role="execute"]
----
oc logs -n backstage -l app.kubernetes.io/component=database --tail=20
----
...
[source,bash,role="execute"]
----
curl -s $(oc get route backstage-developer-hub -n backstage -o jsonpath='https://{.spec.host}')/healthcheck
----
...
[source,bash,role="execute"]
----
oc logs -n rhdh-operator deployment/rhdh-operator-controller-manager --tail=50
----
