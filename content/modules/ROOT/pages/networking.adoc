...
Wait for the ALB to provision (2-3 minutes). First, get the ALB hostname:

[source,bash,role="execute"]
----
oc get ingress weathernow-alb -n app-management
----
...
weathernow-alb   alb     *       k8s-appmanag-weathern-xxxxx.us-east-2.elb.amazonaws.com   80      2m
----

NOTE: The `HOSTS` column showing `*` is expected (wildcard matching). The `ADDRESS` column contains your ALB hostname.

Verify the ALB is ready by testing the endpoint:

[source,bash,role="execute"]
----
ALB_HOST=$(oc get ingress weathernow-alb -n app-management -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
curl -s http://$ALB_HOST | head -20
----

When you see HTML output from the WeatherNow app, the ALB is ready.
...
Expected: HTML output from the WeatherNow app (starts with `<!DOCTYPE html>`)
...
To clean up all resources created in this module, run:
...
# Delete egress firewall (if created)
oc delete egressfirewall default -n app-management --ignore-not-found
